...
int count_solutions(int board[][9], int current_pos[], int* count, int count_all) {
    for (size_t i = 1; i <= 9; i++) {
        if (is_valid(board, current_pos, i)) {
            board[current_pos[0]][current_pos[1]] = i;
            ...
            if (count_solutions(board, next_empty_pos, count, count_all) > 1) {
                if (!count_all) {
                    board[current_pos[0]][current_pos[1]] = 0;
                    return *(count);
                } 
...
int main() {
    ...
    int zeros[9][9] = {0};
    int temp = next_empty_cell(zeros);
    int next_empty_pos[2] = {temp / 10, temp % 10};
    int count = 0;
    count_solutions(zeros, next_empty_pos, &count, 0);
    printf("%d\n", count);
    print_board(zeros);
}